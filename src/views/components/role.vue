    ...
    <div class="content">
      <div class="filter-container">
        <el-form :model="qc" :inline=true>
          <el-input style="width: 200px;" size="small" class="filter-item" placeholder="角色名称" v-model="qc.name"
                    v-if="btnAuth.buttonRoleQuery">
          </el-input>
          <el-button class="filter-item" size="small" type="primary" icon="el-icon-search" @click="query"
                     v-if="btnAuth.buttonRoleQuery">查询
          </el-button>
          <router-link to="/components/roleListInsert">
            <el-button class="filter-item" size="small" type="primary" icon="el-icon-plus" v-if="btnAuth.buttonRoleAdd">
              添加
            </el-button>
          </router-link>
          <el-button class="filter-item" type="primary" size="small" icon="el-icon-close" @click="reset"
                     v-if="btnAuth.buttonRoleQuery">重置
          </el-button>
        </el-form>
        <!--数据列表-->
        <el-table
          ...
          >
            <template slot-scope="scope">
              <router-link :to="{path:'/components/roleListUpdate',query:{id:scope.row.id}}">
                <el-button type="text" icon="el-icon-edit" class="btnStyle" size="small" title="编辑"
                           v-if="btnAuth.buttonRoleEditor"></el-button>
              </router-link>
              <el-button type="text" @click="del(scope.row.id)" icon="el-icon-delete" size="small" class="btnStyle"
                         title="删除" v-if="btnAuth.buttonRoleDel"></el-button>
              <router-link :to="{path:'/components/allocatingMenu',query:{id:scope.row.id}}">
                <el-button type="text" icon="el-icon-menu" class="btnStyle" size="small" title="分配权限"
