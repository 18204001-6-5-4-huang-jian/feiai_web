...
<div class="pics">
  <ul class='fileList clearfix'>
    <li class="noimg" v-if="!fileList1">
      暂无图片
    </li>
    <li  class="fl" v-for="item in fileList1">
  ...
  export default {
    ...
    mounted(){
        setTimeout(()=>{
          ...
          for(let i =0;i<arr.length;i++){
            var obj ={
              name:arr[i],
              url:SERVER_NAME + '/ct/find/image/path?id='+arr[i]
            }
    ...
    methods:{
      zoom(item){
        ...
        this.index=this.fileList1.indexOf(item);
        if(this.index==0){
          this.showA=false
        }else{
          this.showA=true
        }
        if(this.index+1==this.fileList1.length){
          this.showB=false
        }else{
          this.showB=true
        }
     ...
     //前一张
      pre(){
        ...
        this.showB=true;
        this.dialogImageUrl=this.fileList1[this.index].url;
