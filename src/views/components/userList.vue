    ...
    <div class="content">
      <div class="filter-container">
        <el-form :model="qc" :inline=true>
          <el-input style="width: 200px;" size="small" class="filter-item" placeholder="用户昵称" v-model="qc.nickName"
                    v-if="btnAuth.buttonUserQuery">
          </el-input>
          <el-input style="width: 200px;" size="small" class="filter-item" placeholder="登录名" v-model="qc.loginName"
                    v-if="btnAuth.buttonUserQuery">
          </el-input>
          <el-button class="filter-item" size="small" type="primary" icon="el-icon-search" @click="query"
                     v-if="btnAuth.buttonUserQuery">查询
          </el-button>
          <router-link to="/components/userListInsert">
            <el-button type="primary" size="small" icon="el-icon-plus" class="filter-item" v-if="btnAuth.buttonUserAdd">
              添加
            </el-button>
          </router-link>
          <el-button class="filter-item" type="primary" size="small" icon="el-icon-close" @click="reset"
                     v-if="btnAuth.buttonUserQuery">重置
          </el-button>
        </el-form>
      </div>
      <div>
        ...
        <el-table
          ...
          >
            <template slot-scope="scope">
              <router-link :to="{path:'/components/userListUpdate',query:{id:scope.row.id}}">
                <el-button class="btnStyle" type="text" icon="el-icon-edit" size="small" title="编辑"
                           v-if="btnAuth.buttonUserEditor"></el-button>
              </router-link>
              <el-button class="btnStyle" type="text" @click="del(scope.row.id)" icon="el-icon-delete" size="small"
                         title="删除" v-if="btnAuth.buttonUserDel"></el-button>
              <router-link :to="{path:'/components/allocatingRole',query:{id:scope.row.id}}">
                <el-button class="btnStyle" type="text" icon="el-icon-setting" size="small" title="分配角色"
                           v-if="btnAuth.buttonUserRoleDis"></el-button>
              </router-link>
            </template>
          </el-table-column>
        </el-table>
