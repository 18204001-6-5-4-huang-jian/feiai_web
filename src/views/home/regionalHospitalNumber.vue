<template>
  <div v-if="regionalHospitalNumber_page" class="form-wrapper">
    <router-link to="/booking/numberSource">
      <el-button size="mini" class="return">返 回</el-button>
    </router-link>
    <el-form :model="insertForm" :rules="rules" ref="insertForm" class="form">
      <div class="clearfloat">
        <el-form-item label="放号类型:" :label-width="formLabelWidth" class="formItemWidth" prop="allocationType">
          <el-select v-model="insertForm.allocationType" clearable placeholder="" :size="$store.state.style.assemblyModel" style="width: 220px;" class="filter-item">
      ...
      <div class="clearfloat">
        <el-form-item label="预约时间:" :label-width="formLabelWidth" prop="reservationDate" class="formItemWidth">
          <el-date-picker
            ...
            placeholder="选择日期"
            :size="$store.state.style.assemblyModel" style="width: 220px;"
            ref="surveyDate"
        ...
        <el-form-item label="" prop="startTime" class="time-width time-margin">
          <el-select v-model="insertForm.startTime" placeholder="开始时间" @change="timeChange" :size="$store.state.style.assemblyModel">
            <el-option
        ...
        <el-form-item label="" prop="endTime" class="time-width">
          <el-select v-model="insertForm.endTime" placeholder="结束时间" :size="$store.state.style.assemblyModel" >
            <el-option
      ...
      <div class="clearfloat">
        <el-form-item label="就诊科室:" :label-width="formLabelWidth" class="formItemWidth" prop="examinationPlace">
          <el-input v-model="insertForm.examinationPlace" auto-complete="off"  :size="$store.state.style.assemblyModel" style="width: 220px;"
          ></el-input>
        </el-form-item>
      </div>
      <div class="relative-div">
        <div class="clearfloat " v-for="item in insertForm.bookingList">
          <el-form-item label="选择社区:" :label-width="formLabelWidth" class="formItemWidth">
            <el-select v-model="item.deptId" placeholder="请选择" :size="$store.state.style.assemblyModel" style="width: 220px;">
              <el-option
          ...
          <el-form-item label="设置人数:" label-width="100px" class="setting-form-width">
            <el-input v-model="item.num" auto-complete="off" ref="num" :size="$store.state.style.assemblyModel"></el-input>
          </el-form-item>
        </div>
      ...
      <div class="clearfloat">
        <el-form-item label="提交员签字:" :label-width="formLabelWidth" prop="signature" class="formItemWidth">
          <el-input v-model="insertForm.signature" auto-complete="off" size="small" :size="$store.state.style.assemblyModel" style="width: 220px;"
  ...
  export default {
    ...
    methods: {
      ...
      plusCommunitys(){
        ...
        if (this.insertForm.bookingList.length + 1 > this.options.length) {
          this.$message({
            type: 'warning',
          ...
          obj.deptId = ''
          obj.num = ''
          this.insertForm.bookingList.push(obj)
        }

      },
      timeChange(value){
        if (value && this.insertForm.endTime){
            if(value > this.insertForm.endTime){
                this.insertForm.endTime = ''
            }
        }
        var a = ''
      ...
      //减去一条社区
      cutCommunitys(){
        if (this.insertForm.bookingList.length < 2) {
          this.$message({
            type: 'warning',
            message: '至少添加一个社区'
          })
        } else {
          this.insertForm.bookingList.pop();
        }
      },
      focus(){
      ...
      add(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            for (let i = 0; i < this.insertForm.bookingList.length; i++) {
              if (this.insertForm.bookingList[i].deptId == "" || this.insertForm.bookingList[i].num == "") {
                this.$message({
                  type: 'warning',
                  message: '社区或人数不能为空'
                })
      ...
      getTable(){
        $axios_http({
          url: "/allocation/record/find/tbr",
          data: {},
          vueObj: this
        }).then((res) => {
          this.queryResult = res.data.result
        })
      },
      query(formName) {
        $axios_http({
          url: "/allocation/record/find/communityDepts",
          data: {},
          vueObj: this
        }).then((res) => {
