  ...
  <div slot="right" class="content-page" v-if="CTBooking_page">
    ...
    <div class="content-page-table">
      <div class="table-btn-group">
        <el-button :size="$store.state.style.assemblyModel" type="primary" icon="el-icon-plus" @click="openDialog('3')" v-if="btnAuth.enterBookingButton2">
          新增
        </el-button>
      ...
      <el-tabs v-model="activeName" type="card" style="margin-top: 20px;" @tab-click="handleClick">
        <el-tab-pane label="未预约" name="first">
          ...
          <el-table
            ...
            <el-table-column
              label="操作"
              v-if="btnAuth.enterBookingButton2"
            >
              <template slot-scope="scope">
                <el-button type="text" :style="{'font-size':$store.state.style.fontSize18}" :size="$store.state.style.assemblyModel" @click="openDialog('2',scope.row)" v-if="btnAuth.enterBookingButton2">立即预约</el-button>
              </template>
            </el-table-column>
          </el-table>
          <!--分页栏-->
        ...
        <el-tab-pane label="已预约" name="second">
          ...
          <el-table
            ...
            >
              <template slot-scope="scope">
                <el-button type="text" :style="{'font-size':$store.state.style.fontSize18}" :size="$store.state.style.assemblyModel" @click="openDialog('1',scope.row)" v-if="scope.row.source=='SYSTEM'&& btnAuth.enterBookingButton3&&scope.row.overdue!=1">取消预约</el-button>
                <router-link :to="{name:'reservations',query:{type:2,id:scope.row.id,}}" v-if="scope.row.source=='SYSTEM'&&btnAuth.enterBookingButton4">
                  <el-button type="text" :style="{'font-size':$store.state.style.fontSize18}" :size="$store.state.style.assemblyModel" >查看预约单</el-button>
                </router-link>
              </template>
            </el-table-column>
          </el-table>
  ...
  export default {
    ...
    data () {
      return {
        ...
        btnAuth: {
          enterBookingButton1:false,//肠镜预约新增
          enterBookingButton2:false,//肠镜未预约立即预约
          enterBookingButton3:false,//肠镜已预约取消预约
          enterBookingButton4:false,//肠镜已预约查看预约单
        },
    ...
    methods: {
      ...
      openDialog(count, row){
        if (count == 1) {
          ...
          this.bookingForm.bookingType = 2
          $axios_http({
            url: "/index/booking/info/list",
            data: {
                flag:2
      ...
      query(){
        let _url;
        if(this.activeName=='first'){
          _url="/bookIngInfoRestful/bookinfo/bookinfoESNolist"
        }else if(this.activeName=='second'){
          _url="/bookIngInfoRestful/bookinfo/bookinfoESYESlist"
        }
        $axios_http({
          url: _url,
          data: this.qc,
      ...
      query1(){
        $axios_http({
          url: "/bookIngInfoRestful/bookinfo/bookinfoESYESlist",
          data: this.qc,
          vueObj: this
        }).then((res) => {
          this.queryResult1.tableData = res.data.result.list;
